...
public class CustomTree extends AbstractList<String> implements Cloneable, Serializable {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (newEntry == entry) {
                if (i < size - 1) {
                    ...
                    // If all the elements to the right are to be deleted, we'll move up the tree:
    ...
    private Entry<String> getUndelRoot(Entry<String> entry, List<Entry<String>> delList) {
        ...
        if (entry.lineNumber == 0) {   // We reached the root and return it.
            return entry;
        }
    ...
    private class SolutionIterator implements Iterator<String> {
        ...
        volatile int index, lastReturned = -1; // lastReturned - index of the last returned element, -1 if the node doesn't exist.
